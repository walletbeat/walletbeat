<script lang="ts">
	...
	let account = $state<ReturnType<typeof getAccount> | null>(null)
	...
	function updateSIWEMessage() {
		const siweSig = testSignatures.find(signature => signature.id === 'siwe-1');
...
Issued At: ${new Date().toISOString()}`;
	...
	function updatePermitOwner() {
		const permitTest = scamAlertTests.find(test => test.id === 'allow-infinite-permit');
	...
	function discoverProviders() {
		...
		window.addEventListener('eip6963:announceProvider', (event: Event) => {
			...
			const exists = stepTestState.discoveredProviders.some(provider => provider.uuid === info.uuid);
	...
	function showFinalResults() {
		...
		const allPassed = allResults.every(result => result.status === 'passed' || result.status === 'partial');
		const hasPartial = allResults.some(result => result.status === 'partial');
	...
	<div class="content-wrapper" data-row="gap-6">
		...
		<div class="main-content">
			...
			{:else if uiState.activeTab === 'eip-support'}
				...
				/>
			{:else if uiState.activeTab === 'scam-alerts'}
				{@const selectedScamAlert = scamAlertTests.find(test => test.id === uiState.selectedScamAlertId)}
				<ScamAlertsTab
