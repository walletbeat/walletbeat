import { PiGlobe, PiGlobeX, PiDeviceMobile, PiDeviceMobileSlash, PiDesktop } from 'react-icons/pi'
...
export default function WalletTable(): React.ReactElement {
	...
	const walletNameColumn = columnHelper.accessor(
		(row: TableRow): CellValue => row.wallet.metadata.displayName,
		{
			id: 'wallet',
			header: 'Wallet',
			cell: createWalletNameCell(false),
		},
	)
			...
			return (
				...
				>
					{row.getVisibleCells().map(cell => (
						<td
							key={cell.id}
							className={`px-4 py-2 dark:text-gray-200 ${cell.column.id === 'rank' ? 'text-center' : ''
								}`}
						>
							...
							<tr className="bg-tertiary" key={headerGroup.id}>
								{headerGroup.headers.map(header => {
									...
									return (
										<th
											key={header.id}
											className={`px-4 py-2 text-[14px] text-secondary ${header.column.id === 'rank' ? 'text-center' : 'text-left'
