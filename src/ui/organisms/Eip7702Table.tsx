...
import type { EVMAddress, NativeAccountAbstractionContract, SmartWalletContract } from '@/schema/contracts'
...
interface TableRow {
	...
	contract: SmartWalletContract | NativeAccountAbstractionContract | 'UNKNOWN' | 'NONE'
	sortPriority: number
}
...
export default function Eip7702Table(): React.JSX.Element {
	...
	const walletContractColumn = columnHelper.display({
		...
		cell: (info: CellContext<TableRow, unknown>): React.ReactNode => {
			...
			// Handle NativeAccountAbstractionContract which doesn't have an address property
	...
	const walletContractSource = columnHelper.display({
		...
		cell: (info: CellContext<TableRow, unknown>): React.ReactNode => {
			...
			// Handle NativeAccountAbstractionContract which doesn't have sourceCode property
			if ('chain' in contract) {
				return <span style={{ opacity: 0.5 }}>N/A</span>
			}
