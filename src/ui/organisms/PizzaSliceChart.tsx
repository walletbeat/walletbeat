...
export const PizzaSliceChart = <Vs extends ValueSet>({
	...
	const createSlices = (): React.ReactNode[] => {
		...
		return mapNonExemptGroupAttributes(evalGroup, (evalAttr, index) => {
			...
			const x1 = Number.parseFloat((centerX + radius * Math.cos(startRad)).toFixed(precision))
			const y1 = Number.parseFloat((centerY + radius * Math.sin(startRad)).toFixed(precision))
			const x2 = Number.parseFloat((centerX + radius * Math.cos(endRad)).toFixed(precision))
			const y2 = Number.parseFloat((centerY + radius * Math.sin(endRad)).toFixed(precision))
