...
export function LicenseDetails({ wallet, value }: LicenseDetailsProps): React.JSX.Element {
	if (value.license === License.PROPRIETARY) {
		return (
			<Typography variant='body2'>
				{wallet.metadata.displayName} is licensed under a proprietary non-open-source
				license.
			</Typography>
		);
	}
	...
	if (value.license === License.UNLICENSED_VISIBLE) {
		return (
			<Typography variant='body2'>
				{wallet.metadata.displayName} has no visible license information. Consequently, it
				should be assumed to be proprietary (not open-source).
			</Typography>
		);
	}
