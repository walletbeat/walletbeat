...
function noPasskeyImplementation(): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
			...
			shortExplanation: mdSentence(`{{WALLET_NAME}} does not implement passkeys or does not use a recognized verification library.`),
			library: PasskeyVerificationLibrary.NONE,
			__brand: brand,
		},
		details: paragraph(`{{WALLET_NAME}} either does not implement passkeys or does not use a recognized verification library for P256/R1 curve operations. Passkeys provide a more secure authentication method than traditional passwords, but proper implementation is crucial for security.`),
		howToImprove: mdParagraph(`{{WALLET_NAME}} should implement passkeys using a well-audited verification library such as [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib) 159K gas.`),
	}
}

function otherPasskeyImplementation(support: PasskeyVerificationSupport): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
			id: 'other_passkey_implementation',
			rating: Rating.PARTIAL,
			displayName: 'Basic passkey implementation',
			shortExplanation: mdSentence(`{{WALLET_NAME}} implements passkeys with a less common verification library.`),
			library: PasskeyVerificationLibrary.OTHER,
			libraryUrl: support.libraryUrl,
			__brand: brand,
		},
		details: paragraph(`{{WALLET_NAME}} implements passkeys using a less common verification library. While this provides better security than no passkey support, using a well-audited and widely recognized library would provide stronger security guarantees.`),
		howToImprove: mdParagraph(`{{WALLET_NAME}} should consider upgrading to a more widely recognized and audited verification library such as [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib) (159K gas).`),
	}
}

function freshCryptoLibImplementation(support: PasskeyVerificationSupport): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
			id: 'fresh_crypto_lib_implementation',
			rating: Rating.PARTIAL,
			displayName: 'outdated passkey verification implementation',
			shortExplanation: mdSentence(`{{WALLET_NAME}} implements passkeys using [Fresh Crypto Lib](https://github.com/rdubois-crypto/FreshCryptoLib).`),
		...
		details: mdParagraph(`{{WALLET_NAME}} implements passkeys using [Fresh Crypto Lib](https://github.com/rdubois-crypto/FreshCryptoLib). While this is a well-regarded library, it has not undergone as extensive auditing and testing as [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib).`),
		howToImprove: mdParagraph(`{{WALLET_NAME}} should consider upgrading to [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib) (159K gas), which has undergone more extensive [auditing and testing.](https://github.com/get-smooth/crypto-lib/tree/main/doc/Audits)`),
	}
}

function smoothCryptoLibImplementation(support: PasskeyVerificationSupport): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
			id: 'smooth_crypto_lib_implementation',
			rating: Rating.PASS,
			displayName: 'Audited passkey implementation SCL',
			shortExplanation: mdSentence(`{{WALLET_NAME}} implements passkeys using [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib).`),
		...
		details: mdParagraph(`{{WALLET_NAME}} implements passkeys using [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib), at 159K this the most gas-efficient ( and [triple audited](https://github.com/get-smooth/crypto-lib/tree/main/doc/Audits) verification library for P256/R1 curve operations.`),
		howToImprove: undefined,
	}
}

function daimoP256VerifierImplementation(support: PasskeyVerificationSupport): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
			id: 'daimo_p256_verifier_implementation',
			rating: Rating.PASS,
			displayName: 'Audited passkey implementation (Daimo P256 verifier)',
			shortExplanation: mdSentence(`{{WALLET_NAME}} implements passkeys using [Daimo P256 verifier](https://github.com/daimo-eth/p256-verifier).`),
		...
		details: mdParagraph(`{{WALLET_NAME}} implements passkeys using [Daimo P256 verifier](https://github.com/daimo-eth/p256-verifier), a well-audited verification library for P256/R1 curve operations. Costs 330K gas.`),
		howToImprove: undefined,
	}
}

function openZeppelinP256VerifierImplementation(support: PasskeyVerificationSupport): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
			id: 'open_zeppelin_p256_verifier_implementation',
			rating: Rating.PASS,
			displayName: 'Audited passkey implementation (OpenZeppelin P256 verifier)',
			shortExplanation: mdSentence(`{{WALLET_NAME}} implements passkeys using [OpenZeppelin P256 verifier.](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/cryptography/P256.sol)`),
...
function webAuthnSolImplementation(support: PasskeyVerificationSupport): Evaluation<PasskeyImplementationValue> {
	return {
		value: {
		...
		howToImprove: mdParagraph(`{{WALLET_NAME}} could improve by updating the fallback mechanism to use Smooth Crypto Library instead of FreshCryptoLib for better performance and security.`),
	}
}
...
export const passkeyImplementation: Attribute<PasskeyImplementationValue> = {
	...
	ratingScale: {
		...
		pass: [
			exampleRating(
				mdParagraph(`The wallet implements passkeys using [Smooth Crypto Lib](https://github.com/get-smooth/crypto-lib), the most gas-efficient and triple-audited verification library for P256/R1 curve operations.`),
			...
			exampleRating(
				mdParagraph(`The wallet implements passkeys using [Daimo P256 verifier](https://github.com/daimo-eth/p256-verifier), which is well-audited and reasonably gas-efficient.`),
			...
			exampleRating(
				mdParagraph(`The wallet implements passkeys using [OpenZeppelin P256 verifier](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/cryptography/P256.sol), a well-audited verification library from a respected team.`),
		...
		partial: [
			exampleRating(
				mdParagraph(`The wallet implements passkeys using [Fresh Crypto Lib](https://github.com/rdubois-crypto/FreshCryptoLib), a well-regarded but suboptimal verification library for P256/R1 curve operations.`),
			...
			exampleRating(
				mdParagraph(`The wallet implements passkeys using [WebAuthn.sol](https://github.com/base/webauthn-sol), which builds on Daimo's WebAuthn.sol but falls back to the FreshCryptoLib.`),
			...
			exampleRating(
				mdSentence(`The wallet implements passkeys using a non audited / less common verification library.`),
		...
		fail: [
			exampleRating(
				mdParagraph(`The wallet does not implement passkeys or does not use a recognized verification library for P256/R1 curve operations.`),
	...
	evaluate: (features: ResolvedFeatures): Evaluation<PasskeyImplementationValue> => {
		...
		if (features.variant === Variant.HARDWARE) {
			return exempt(
				...
				sentence(`This attribute is not applicable for {{WALLET_NAME}} as it is a hardware wallet and doesn't use passkeys.`),
				brand,
				{ library: PasskeyVerificationLibrary.NONE },
			)
		}
		...
		if (passkeyVerification.library === PasskeyVerificationLibrary.NONE) {
			return exempt(
				passkeyImplementation,
				sentence(`This attribute is not applicable for {{WALLET_NAME}} as it doesn't implement passkeys.`),
				brand,
				{ library: PasskeyVerificationLibrary.NONE },
			)
		}
