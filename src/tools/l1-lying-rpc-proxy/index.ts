...
function createHandler(upstream: string, multiplier: bigint) {
  return async (req: http.IncomingMessage, res: http.ServerResponse): Promise<void> => {
    ...
    try {
      payload = JSON.parse(rawBody.toString('utf8')) as JsonRpcRequest | JsonRpcRequest[]
    } catch {
    ...
    try {
      ...
      if (anyLie) {
        console.error('[proxy] upstream returned non-JSON for a request we need to lie about')
        res.writeHead(502, { 'Content-Type': 'application/json' })
    ...
    // Normalise to array for processing
    ...
    // If not, we cannot safely correlate lie flags to responses.
    if (responses.length !== requests.length) {
      console.error(
        `[proxy] upstream returned ${responses.length} response(s) for ${requests.length} request(s)`,
      )
      res.writeHead(502, { 'Content-Type': 'application/json' })
      res.end(
        jsonError(
          `Upstream response count mismatch: expected ${requests.length}, got ${responses.length}`,
        ),
      )
      return
    }
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      if (resp && typeof resp.result === 'string' && resp.result.startsWith('0x')) {
        const originalBalance = resp.result
        const liedBalance = lieAboutBalance(resp.result, multiplier)
        resp.result = liedBalance
        console.log(
          `[proxy] LYING: ERC-20 balanceOf result changed ${originalBalance} → ${liedBalance} (×${multiplier})`,
        )
      }
    }
...
server.listen(port, () => {
  ...
  console.log(`[l1-lying-rpc-proxy] ERC-20 balance multiplier: ×${multiplier}`)
