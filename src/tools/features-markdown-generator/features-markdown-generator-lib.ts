...
export async function generateMarkdown(config: FeaturesMarkdownConfig): Promise<string> {
	...
	const raw =
		'---\n' +
		'title: Walletbeat Feature Types\n' +
		"description: 'Autogenerated feature TypeScript documentation'\n" +
		'---\n' +
		'\n' +
...
export async function featuresMarkdownUpdate(config: FeaturesMarkdownConfig): Promise<void> {
	...
	if (config.test) {
		if (!fs.existsSync(config.outputPath)) {
			throw new Error(
				'resources/docs/features/features.md does not exist. Run `pnpm fix` to generate it.',
			)
		}
		...
		if (existingContent.trim() !== markdownContent.trim()) {
			throw new Error(
				'resources/docs/features/features.md is out of sync with the TypeScript source. Run `pnpm fix` to regenerate.',
			)
		}
	} else {
		if (fs.existsSync(config.outputPath)) {
