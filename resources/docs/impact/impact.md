---
title: 'Walletbeat impact log'
description: "A lot of actions that wallets have taken as a result of Walletbeat's actions or existence."
---

- **2025-12**: Walletbeat publishes the L1 Provider Independence Test: https://x.com/walletbeat/status/2002818293741301777
  - Reaction: Ambire fixes its L1 RPC configuration option to be available at wallet setup time: https://x.com/walletbeat/status/2017683277730001010 / https://x.com/Ivshti/status/2017967726782783683
    - 2026-07: Faced with a regression, Ambire commits to an integration test to avoid further regressions: https://x.com/Ivshti/status/2075451993838002660
    - Reaction: MetaMask (quietly) fixes its hard-reliance on a Consensys token metadata service when sending tokens: https://x.com/walletbeat/status/2066296900785942850
- **2026-07**: Walletbeat contributor 0xMattmatt shows how multiple wallets handle a malicious Ethereum transaction that caused a user to lose ~30k USD: https://x.com/0xMattmatt/status/2081106605463289971
  - **Reaction**: Ambire fixes this in their next release: https://x.com/borislavItskovv/status/2081582575790006360
    - **Reaction**: WalletChan fixes this in their next release: https://x.com/walletbeat/status/2082293464071373094
