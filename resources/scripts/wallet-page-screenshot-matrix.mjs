...
const viewportSizes = (
	process.env['WALLETBEAT_MATRIX_VIEW' + 'PORTS'] ??
	'390x568,390x844,480x844,768x900,864x568,864x900,1024x900,1025x900,1280x900,1281x900,1440x1200'
)
	.split(',')
	.map(viewport => {
...
const screenshotCount = engineNames.length * viewportSizes.length * states.length
const screenshots = []
...
<p>${engineNames.join(' · ')}. Viewport sizes: ${viewportSizes.map(({ width, height }) => `${width}×${height}`).join(', ')}px.</p>
<table>
