...
import { assertNonEmptyArray, isNonEmptyArray } from '@/types/utils/non-empty'
						...
						case 'pass-fail':
							;(() => {
								const passRatings = assertNonEmptyArray(
									normalizeRatings(attribute.ratingScale.pass),
								)
								...
								const failRatings = assertNonEmptyArray(
									normalizeRatings(attribute.ratingScale.fail),
								)
								const checkRatings = (
									scale: string,
